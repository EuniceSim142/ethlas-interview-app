...
class SpaceXClient {
   ...
   */
  fetchAllCompanyData() {
    const pathURL = this.#BASEURL + this.#URL_PATHS.COMPANY;

    return fetch(pathURL)
      .then((res) => res.json())
      .then((allData) => this.#createCompanyData(allData));
  }

  /**
   * Fetches company data from API and returns only relevant data.
   * @param {JSON} companyData data from API.
   * @returns relevant company data.
   */
  ...
  /**
   * Fetches dragon data from API and returns only relevant data.
  ...
  /**
   * Fetches rocket data from API and returns only relevant data.
...
// spaceXClient object will be shared throughout the app for data retrieval
